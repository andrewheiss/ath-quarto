...
The intercept of this intercept-only model is `r round(logit0_intercept, 3)` in logit units, whatever that means. We can convert this to a probability though:
...
The effect here is small: a one-unit change in polyarchy (i.e. moving from a 56 to a 57) is associated with a 0.00119 percentage point increase in the proportion of women MPs, on average. That's not *huge*, but a one-unit change in polyarchy also isn't huge. If a country moves 10 points from 56 to 66, for instance, there'd be a ≈10x increase in the effect (0.00119 * 10, or `r 0.00119 * 10`). This is apparent in the predicted proportion when polyarchy is set to 56 and 66: 0.2026564 - 0.1914267 = `r 0.2026564 - 0.1914267`
...
```{r model-bayes-beta-basic, cache=TRUE, results="hide"}
...
```{r model-bayes-beta-complex, cache=TRUE, results="hide"}
...
```{r beta-bayes-polyarchy-overall}
...
```{r model-bayes-zoi-int-only, cache=TRUE, results="hide"}
...
```{r model-bayes-zi, cache=TRUE, results="hide"}
model_beta_zi <- brm(
  bf(prop_fem ~ quota,
     phi ~ quota,
     zi ~ quota),
...
```{r model-fancy-bayes, cache=TRUE, results="hide", warning=FALSE}
fancy_model <- brm(
  fancy_formula,
  data = vdem_clean,
  family = zero_inflated_beta(),
