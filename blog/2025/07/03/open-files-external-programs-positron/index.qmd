...
::: {.callout-note title="Update!"}
In March/April 2026, Typora created its own [VS Code extension](https://support.typora.io/Open-in-Typora-VS-Code-Extension/) (here's [the Open VSX version](https://open-vsx.org/extension/typora/typora-open-in-typora)) for opening Markdown files in Typora, so you don't need to set up a custom task for that any more. I now have this set as my keyboard shortcut:

```{.json filename="keybindings.json"}
{
    "key": "ctrl+shift+alt+cmd+t",
    "command": "typora.openInTypora"
}
```

Tasks are still a useful way to open any other type of file in any other external program though!
:::
