...
cookie_mfx <- ggplot(cookie_plot_data, aes(x = cookies, y = value)) +
  ...
  geom_abline(aes(slope = slope, intercept = intercept_real),
              size = 0.25, color = clrs[4], linetype = "21") +
...
```{r get-clean-data, cache=TRUE, R.options = list(width=90)}
...
We can also use summarizing methods built in to **marginaleffects** by using the `by` argument in `marginaleffects()`. This is the better option, since it does some tricky standard error calculations behind the scenes:
...
cfct <- r"{marginaleffects(
   model, 
   newdata = datagrid(some_x = c(10, 20), 
   grid_type = "counterfactual")
)}"

mer_mfx <- r"{marginaleffects(
   model, 
   newdata = datagrid(some_x = c(10, 20))
)}"
...
tribble(
  ...
  "Average marginal effects (AME)", "Generate predictions for each row of the original data, then collapse to averages", wrap_r("marginaleffects(model)"), "Not supported",
  "Group average marginal effects (G-AME)", "Generate predictions for each row of the original data, then collapse to grouped averages", wrap_r('marginaleffects(model, by = "some_group")'), "Not supported",
  "Marginal effects at the mean (MEM)", "Collapse data to averages, then generate predictions using those averages", wrap_r('marginaleffects(model, newdata = "mean")'), wrap_r('emtrends(model, ...)'),
  ...
  "Average marginal effects at counterfactual user-specified values", "Create multiple copies of the original data with some columns set to specific values, then generate predictions for each row of each copy of the original data, then collapse to averages", wrap_r(cfct), "Not supported"
) |> 
