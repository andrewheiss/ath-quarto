...
We can use a [mosaic plot](https://en.wikipedia.org/wiki/Mosaic_plot) (also known as a [Marimekko chart](https://datavizcatalogue.com/methods/marimekko_chart.html)) to visualize both the distribution of proportions of responses (Rarely, Sometimes, Often) and the proportions of respondents from each country (Mexico and the United States). We can use the {ggmosaic} package to create ggplot-flavored mosaic plots:
...
Until {brms} 2.19.something, there wasn't an official beta-binomial distribution for {brms}, but it was used as [the example for creating your own custom family](https://cran.r-project.org/web/packages/brms/vignettes/brms_customfamilies.html). However, when I try to use the new `family = beta_binomial()`, the results I get are wildly off from both raw Stan and from `family = binomial()` in {brms}, and I don't know why :shrug:. Someday I'll be smart enough to figure out how to use it, but for now, everything up above works great.

```{r q1-model-beta-binomial, eval=FALSE}
# This doesn't work!
often_comics_model <- brm(
  bf(n | trials(total) ~ 0 + country),
  data = often_comics_only,
...
Let's first extract the aggregated data we'll work with—newspaper frequcny in the United States only:
